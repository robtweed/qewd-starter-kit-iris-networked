...
## Edit the *jsdb_shell.js* File

Before you begin, you'll need to edit the IRIS database settings in the *jsdb_shell.js*
file.  Using your favourite editor, find the lines:
...
Change the values to match your *config.json* file's settings.  In particular, make sure it's
using the correct values for *host*, *tcp_port* and *password* to allow it to make a network
connection to IRIS via the *mgsi* SuperServer.
...
If you've set the correct IRIS database connection values, you should now see something like:
...
If it hangs and doesn't come back with these two critical lines:

        undefined
        >

then it means that it's unable to connect to your IRIS database.  Check the IRIS
connection settings in the *jsdb_shell.js* Module file and then try again - you'll need
to use multiple *CTRL & C*s to exit the Node.js REPL and then restart it.

However, if the *QEWD-JSdb shell* module loaded correctly, congratulations!  You can
...
Using your favourite IRIS tool, check in the *USER* namespace for a Global named *^demo*.
You should see this:
...
and if you check for the Global in IRIS, you'll see that it has disappeared.
...
When you looked in IRIS, you saw that this had caused a Global Node to be stored in the database:
...
and you'll see that new value appearing as a Global in IRIS.
...
In IRIS, you'll now see the following Global Nodes:
...
Check in IRIS and you should now see the following in the ^demo Global, eg:
...
In IRIS you'll see this has been created:
...
and if you look at the physical *^demo* Global in IRIS, you can see that we've iterated through the entire
Global:
...
Of course, if you look in IRIS, the names, because they're defined as Global subscripts,
...
Take a look at the *^demo* Global in IRIS and you'll see how it's stored the array:
...
You'll see this in IRIS, saved as:
...
Look in IRIS and you'll see that this has resulted in these records being added to the database:
...
Check in IRIS.  You should see that it's saved it as:
...
Look in IRIS and you'll see that an *id* of 2 was assigned to this, and the *next_id*
...
If you look in IRIS, you should see that the data record has correctly changed:
